...
export async function relatorioPedidosFechados(parametros: RelatorioImpressaoFiltroDTO) {
  const doc = new DefaultReport({ parametros: parametros, titulo: 'Relatório de Pedidos Fechados' });
  const pedidos = await pedidoRepository.pedidosFechados(parametros);
  ...
  function montaPedido(pedido: Pedido, y: number) {
    ...
    doc
      .font('Helvetica', 10)
      .text(`Fechado em ${dataCadastroFormatada}`, 0, y, { align: 'center' });
